...
export class ViewBillsComponent implements OnInit, OnDestroy {
  ...
  constructor(private viewBillsService: ViewBillsService) {}
  ...
  // ════════════════════════════════════════
  // LOAD — filtered by CURRENT_USER_ID
  // ════════════════════════════════════════
  ...
  loadBills(): void {
    ...
    // ← getByUserId filters bills where bill.userId === 1
  ...
  updateStats(): void {
    const totalRevenue = this.allBills.reduce((s, b) => s + this.calcTotal(b), 0);
  ...
  downloadPdf(bill: Bill, event?: Event): void {
    ...
    doc.text(`Payment: ${bill.payment}`,              14, 68);
  ...
  exportAllExcel(): void {
    ...
    ws['!cols'] = [
      { wch: 16 }, { wch: 22 }, { wch: 14 }, { wch: 10 },
      { wch: 12 }, { wch: 10 }, { wch: 10 }, { wch: 12 }, { wch: 8 }
    ];

    const buf  = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([buf], { type: 'application/octet-stream' });
    saveAs(blob, `FinanceTracker-Bills-${new Date().toISOString().slice(0, 10)}.xlsx`);
    this.showToast(`Exported ${this.filtered.length} bills to Excel`, 'success');
  }
