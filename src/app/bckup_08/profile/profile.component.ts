...
export class ProfileComponent implements OnInit {
  ...
  // ── Restaurant summary (shown on card) ──
  fRestaurant: string = '';
  fGst:        string = '';
  fAddress:    string = '';
  ...
  loadUserFromSession(): void {
    ...
    if (authUser) {
      ...
      this.fFirst   = authUser.userName || '';   // API has no firstName — use userName
      this.fLast    = '';
      this.fRole    = this.normalizeRole(authUser.role);

      // Avatar — check ftProfile first for custom avatar
      const rawProfile = localStorage.getItem('ftProfile');
      if (rawProfile) {
        try {
          const p = JSON.parse(rawProfile);
          // Only take avatar and extra fields from ftProfile
  ...
  loadRestaurants(): void {
    ...
    this.restaurantService.getAll().subscribe({
      next: (list) => {
        ...
        // Show first restaurant in the summary card
        if (list.length > 0) {
  ...
  // ════════════════════════════════════════
  // SAVE PERSONAL INFO
  // Saves to ftProfile (local) + updates
  // userName/email via AuthService.updateUser()
  // ════════════════════════════════════════
  ...
  savePersonal(): void {
    ...
    this.authService.updateUser(userId, {
      ...
      error: (err) => {
        console.error('updateUser API failed:', err);
        // Still update locally even if API fails
      }
    });

    // ── Save extras to ftProfile (phone, dob, bio, avatar) ──
